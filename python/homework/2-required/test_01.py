...
class Test(testlib.TestCase):
    def do_test(self, list_of_xkcd: list[str], k: int, expected: list[int]):
        ...
        if DEBUG:
            ...
            with self.ignored_function("builtins.print"), self.ignored_function(
                "pprint.pprint"
            ), self.forbidden_function("builtins.input"), self.forbidden_function(
                "builtins.eval"
            ), self.check_imports(
                allowed=["program01", "_io", "typing"]
            ), self.timeout(
                TIMEOUT
            ), self.timer(
                TIMEOUT
    ...
    @unpack
    def test_list_of_weigths_to_number(
        self, integers: list[int], expected: int, error1: int
    ):
        import program01 as program
