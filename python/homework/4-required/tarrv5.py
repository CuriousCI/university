...
def most_frequent_chars(filename):
    dizPosOcc = dict()
    dupParole, lmax = operazione(filename)
    contaOccorrenze(dupParole, dizPosOcc, lmax)
    return creaStringa(dizPosOcc, lmax)
...
def operazione(filename):
    ...
    while proxFile not in openedFile:
        with open(filename, encoding="utf8") as f:
            ...
            if filename not in openedFile:
                openedFile[filename] = True
    ...
    return dupParole, lmax


def contaOccorrenze(dupParole, dizPosOcc, lmax):
