...
def sum_recursive(n: int) -> int:
    return n + sum_recursive(n-1) if n else 0
...
def power_recursive(base: int, exp: int) -> int:
    return base * power_recursive(base, exp-1) if exp else 1
...
def reverse_recursive(s: str) -> str:
    ...
    mid = len(s) // 2
    return reverse_recursive(s[mid:]) + reverse_recursive(s[:mid])
...
# return 1 if n == 1 else n + sum([hanoi_moves(n - i) for i in range(1, n+1)])
def hanoi_moves(n: int) -> int:
    return 1 if n == 1 else 2 * hanoi_moves(n - 1) + 1
