...
def img_rotate(img_in: str, theta: float, img_out: str):
    ...
    result = [[(0, 0, 0) for _ in row] for row in image]
    theta = radians(theta)
    width, height = len(image[0]), len(result)

    for index, row in enumerate(image):
        for x, pixel in enumerate(row):
            x -= int(width / 2)
            y = index - int(height / 2) 

            x, y = int(x*cos(theta) + y*sin(theta)), int(-x*sin(theta) + y*cos(theta))
...
def img_colorgrade(img_in: str, t: tuple, c: float, s: float, img_out: str):
    ...
    for y, row in enumerate(image):
        for x, pixel in enumerate(row):
            pixel = tuple(color / 255 for color in pixel)
            pixel = tuple(pixel[c] * t[c] for c in range(3))
            pixel = tuple((pixel[i] - .5) * c + .5 for i in range(3))
            grey = sum(pixel) / 3
            pixel = tuple((pixel[i] - grey) * s + grey for i in range(3))
            pixel = tuple(max(0, min(255, int(255 * color))) for color in pixel)
            image[y][x] = pixel
